...
var findType = findKey.bind(null, 'type');

function deserialize(relationshipsMeta, schema, _ref3) {
  var id = _ref3.id,
  ...
  var attrs = findType(schema) === 'item' && attributes ? Object.keys(attributes).map(function (key) {
    return {
      key: key,
      details: null
    };
...
function deserializeJsonApi(targetSchema, json) {
  if (!json) {
    return json;
  }
  ...
  if (Array.isArray(data)) {
    return data.map(function (item) {
      return deserialize(relationshipsMeta, targetSchema, item);
    });
  }
